...
for f in $( cat runlist.txt ); do
	if ! PGOPTIONS='--client-min-messages=warning' psql -bq -d reports_dev -c '\set ON_ERROR_STOP on' -c 'set search_path = public' -f $f -U runner ; then
		exit 1
	fi
done || exit 1
...
# fi
# for f in $( cat runlist.txt ); do
# 	if ! PGOPTIONS='--client-min-messages=warning' psql -bq -d folio_snapshot -c '\set ON_ERROR_STOP on' -c 'set search_path = folio_reporting,public' -f $f -U runner ; then
# 		exit 1
# 	fi
# done || exit 1
