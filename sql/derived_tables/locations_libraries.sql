DROP TABLE IF EXISTS local.locations_libraries;

CREATE TABLE local.locations_libraries AS
SELECT
...
CREATE INDEX ON local.locations_libraries (campus_id);

CREATE INDEX ON local.locations_libraries (campus_name);

CREATE INDEX ON local.locations_libraries (location_id);

CREATE INDEX ON local.locations_libraries (location_name);

CREATE INDEX ON local.locations_libraries (discovery_display_name);

CREATE INDEX ON local.locations_libraries (library_id);

CREATE INDEX ON local.locations_libraries (library_name);

CREATE INDEX ON local.locations_libraries (institution_id);

CREATE INDEX ON local.locations_libraries (institution_name);

VACUUM ANALYZE local.locations_libraries;
