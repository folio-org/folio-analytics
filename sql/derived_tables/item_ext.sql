...
FROM
    ...
    LEFT JOIN inventory_call_number_types AS item_call_number_type ON items.item_level_call_number_type_id::text = item_call_number_type.id::text
    LEFT JOIN inventory_call_number_types AS effective_call_number_type
        ON (items.data #>> '{effectiveCallNumberComponents,typeID}')::text = effective_call_number_type.id::text;
