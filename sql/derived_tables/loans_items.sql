--Create a derived table that contains all items from loans
--and adds item, location, and other loan-related information
/*Tables included
 circulation_loans
 inventory_items
 ...
 */
--Location names are from the items table
--They show location of the item right now vs. when item was checked out
...
SELECT
    ...
    FROM public.circulation_loans AS cl
	LEFT JOIN public.inventory_items AS ii
		ON cl.item_id = ii.id
	LEFT JOIN public.inventory_material_types AS imt
		ON ii.material_type_id = imt.id
	LEFT JOIN public.circulation_loan_policies AS clp
		ON cl.loan_policy_id = clp.id
	LEFT JOIN public.user_groups AS ug
		ON cl.patron_group_id_at_checkout = ug.id
	LEFT JOIN public.inventory_locations AS iel 
		ON ii.effective_location_id = iel.id 
	LEFT JOIN public.inventory_locations AS ipl 
		ON ii.permanent_location_id = ipl.id 
	LEFT JOIN public.inventory_locations AS itl 
		ON ii.temporary_location_id = itl.id 
	LEFT JOIN public.inventory_locations AS icl 
		ON cl.item_effective_location_id_at_check_out = icl.id
	LEFT JOIN public.inventory_service_points AS ispi
		ON cl.checkin_service_point_id = ispi.id 
	LEFT JOIN public.inventory_service_points AS ispo
		ON cl.checkin_service_point_id = ispo.id 
	LEFT JOIN public.inventory_service_points AS ispt
		ON ii.in_transit_destination_service_point_id = ispt.id 
	LEFT JOIN public.inventory_loan_types AS iltp 
		ON ii.temporary_loan_type_id = iltp.id 
	LEFT JOIN public.inventory_loan_types AS iltt 
		ON ii.permanent_loan_type_id = iltt.id 	
	LEFT JOIN public.feesfines_overdue_fines_policies AS ffo  
		ON cl.overdue_fine_policy_id =ffo.id 
	LEFT JOIN public.feesfines_lost_item_fees_policies AS ffl  
		ON cl.lost_item_policy_id = ffl.id 
	;
