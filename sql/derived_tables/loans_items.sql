...
FROM
    ...
    LEFT JOIN circulation_loan_policies AS clp ON (cl.data #>> '{loanPolicyId}')::uuid = clp.id::uuid
    LEFT JOIN user_groups AS ug ON (cl.data #>> '{patronGroupIdAtCheckout}')::uuid = ug.id::uuid
    LEFT JOIN inventory_locations AS iel ON ii.effective_location_id = iel.id
    LEFT JOIN inventory_locations AS ipl ON (ii.data #>> '{permanentLocationId}')::uuid = ipl.id::uuid
    LEFT JOIN locations_libraries AS ll ON ipl.id = ll.location_id
    LEFT JOIN inventory_locations AS itl ON (ii.data #>> '{temporaryLocationId}')::uuid = itl.id::uuid
    LEFT JOIN inventory_locations AS icl ON (cl.data #>> '{itemEffectiveLocationIdAtCheckOut}')::uuid = icl.id::uuid
