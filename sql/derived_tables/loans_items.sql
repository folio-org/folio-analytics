...
FROM
    ...
    LEFT JOIN circulation_loan_policies AS clp ON cl.loan_policy_id = clp.id
    LEFT JOIN user_groups AS ug ON cl.patron_group_id_at_checkout = ug.id
    ...
    LEFT JOIN inventory_locations AS icl ON cl.item_effective_location_id_at_check_out = icl.id
    ...
    LEFT JOIN feesfines_overdue_fines_policies AS ffo ON cl.overdue_fine_policy_id = ffo.id
    LEFT JOIN feesfines_lost_item_fees_policies AS ffl ON cl.lost_item_policy_id = ffl.id;
