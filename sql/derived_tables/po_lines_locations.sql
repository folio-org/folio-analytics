DROP TABLE IF EXISTS local.nancybolduc_po_lines_locations;
--Create a local table for Purchase Order Line Locations including location quantity and type.
CREATE TABLE local.nancybolduc_po_lines_locations AS
...
FROM pol_location
	LEFT JOIN inventory_locations
		ON inventory_locations.id = pol_location.pol_loc_id
ORDER BY pol_id;
	
CREATE INDEX ON local.nancybolduc_po_lines_locations (pol_id);

CREATE INDEX ON local.nancybolduc_po_lines_locations (pol_loc_id);

CREATE INDEX ON local.nancybolduc_po_lines_locations (pol_loc_qty);

CREATE INDEX ON local.nancybolduc_po_lines_locations (pol_loc_qty_elec);

CREATE INDEX ON local.nancybolduc_po_lines_locations (pol_loc_quant_phys);

CREATE INDEX ON local.nancybolduc_po_lines_locations (location_name);