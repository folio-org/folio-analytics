DROP TABLE IF EXISTS local.instance_alternative_titles;

-- Create a derived table for alternative titles from instance records with the type id and name included.
CREATE TABLE local.instance_alternative_titles AS
SELECT
...
CREATE INDEX ON local.instance_alternative_titles (instance_id);

CREATE INDEX ON local.instance_alternative_titles (instance_hrid);

CREATE INDEX ON local.instance_alternative_titles (alternative_title);

CREATE INDEX ON local.instance_alternative_titles (alternative_title_type_id);

CREATE INDEX ON local.instance_alternative_titles (alternative_title_type_name);
