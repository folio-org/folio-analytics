DROP TABLE if exists local.alternative_titles
--Create a derived table for alternative titles from instance records

CREATE TABLE local.alternative_titles AS
...
CREATE INDEX ON local.alternative_titles (ins_id);

CREATE INDEX ON local.alternative_titles (ins_hrid);

CREATE INDEX ON local.alternative_titles (ins_alternative_title);

CREATE INDEX ON local.alternative_titles (ins_alternative_title_type_id);

CREATE INDEX ON local.alternative_titles (ins_alternative_title_types_name);
