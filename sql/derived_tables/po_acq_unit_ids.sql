DROP TABLE IF EXISTS local.po_acq_unit_ids;

-- These fields in adjustments can be locally defined
CREATE TABLE local.po_acq_unit_ids AS
WITH po_acq_unit AS (
    SELECT
        id AS po_id,
...
CREATE INDEX ON local.po_acq_unit_ids (po_id);

CREATE INDEX ON local.po_acq_unit_ids (po_number);

CREATE INDEX ON local.po_acq_unit_ids (po_acquisition_unit_id);

CREATE INDEX ON local.po_acq_unit_ids (po_acquisition_unit_name);
