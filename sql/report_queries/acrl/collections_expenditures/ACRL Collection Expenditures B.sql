...
--This query does not include transactions that are made at the invoice level that are not prorated. 
--If the total of invoice transactions not distributed to the Invoice Lines is needed (e.g. adjustmetns), then the ACRL Expenditures C should be run jointly. 
--An export to Excel will provide the total amount paid by summing the 'transaction_amount_per_invl_converted' column
...
invl_transac_amount_total AS( --Provides summ of transactions per invoice_line
	SELECT
		invl.id AS invl_id,
