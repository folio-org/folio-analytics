...
FROM
    ...
    LEFT JOIN subquery_items_with_notes AS nn ON li.item_id = nn.item_id
    ...
    LEFT JOIN subquery_instances_with_publication_dates AS pd ON he.instance_id = pd.instance_id
...
WHERE (li.current_item_permanent_location_name = (
        SELECT
            current_item_permanent_location_filter
        FROM
            parameters)
        OR '' = (
            SELECT
    ...
    AND (li.current_item_temporary_location_name = (
            SELECT
                current_item_temporary_location_filter
            FROM
                parameters)
            OR '' = (
                SELECT
        ...
        AND (li.current_item_effective_location_name = (
                SELECT
                    current_item_effective_location_filter
                FROM
                    parameters)
                OR '' = (
                    SELECT
            ...
            AND (li.current_item_permanent_location_library_name = (
                    SELECT
                        current_item_permanent_location_library_filter
                    FROM
                        parameters)
                    OR '' = (
                        SELECT
                ...
                AND (li.current_item_permanent_location_institution_name = (
                        SELECT
                            current_item_permanent_location_institution_filter
                        FROM
                            parameters)
                        OR '' = (
                            SELECT
                    ...
                    AND (li.current_item_permanent_location_campus_name = (
                            SELECT
                                current_item_permanent_location_campus_filter
                            FROM
                                parameters)
                            OR '' = (
                                SELECT
