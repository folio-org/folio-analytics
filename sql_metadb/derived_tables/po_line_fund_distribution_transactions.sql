...
WITH po_line AS (
    SELECT
        ...
        cast(jsonb_extract_path_text(po_line.jsonb, 'cost', 'listUnitPrice') AS decimal(19,4)) AS poline_listUnitPrice,
        cast(jsonb_extract_path_text(po_line.jsonb, 'cost', 'quantityPhysical') AS integer) AS poline_quantityPhysical,
        cast(jsonb_extract_path_text(po_line.jsonb, 'cost', 'listUnitPriceElectronic') AS decimal(19,4)) AS poline_listUnitPriceElectronic,
        cast(jsonb_extract_path_text(po_line.jsonb, 'cost', 'quantityElectronic') AS integer) AS poline_quantityElectronic,
        cast(jsonb_extract_path_text(po_line.jsonb, 'cost', 'additionalCost') AS decimal(19,4)) AS poline_additionalCost,
        ...
        cast(jsonb_extract_path_text(po_line.jsonb, 'cost', 'fyroAdjustmentAmount') AS decimal(19,4)) AS po_line_fyroAdjustmentAmount,
...
transactions AS (
    SELECT
        ...
        cast(jsonb_extract_path_text(finance_transaction.jsonb, 'amount') AS decimal(19,4)) AS transaction_amount,
        cast(jsonb_extract_path_text(finance_transaction.jsonb, 'encumbrance', 'initialAmountEncumbered') AS decimal(19,4)) AS transaction_encumbrance_initial_amount,
        cast(jsonb_extract_path_text(finance_transaction.jsonb, 'encumbrance', 'amountExpended') AS decimal(19,4)) AS transaction_encumbrance_amount_expended,
