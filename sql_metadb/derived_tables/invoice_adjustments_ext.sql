...
WITH invl_total AS (
    SELECT
        inv.id AS inv_id,
        sum(jsonb_extract_path_text(invl.jsonb, 'total')::decimal(19,4)) AS invl_total
    FROM
...
SELECT
    ...
    invl.id AS invl_id,
    coalesce(jsonb_extract_path_text(invl.jsonb, 'total')::decimal(19,4), 0) AS invoice_line_value,
    ...
    CASE WHEN invltotal.invl_total IS NULL
        ...
        0.0000
    ELSE
        round(coalesce(jsonb_extract_path_text(invl.jsonb, 'total')::decimal(19,4), 0) / invltotal.invl_total, 4)
    ...
    CASE WHEN invadj.adjustment_value IS NULL
        ...
        0.0000
    ELSE
        round(invadj.adjustment_value * (coalesce(jsonb_extract_path_text(invl.jsonb, 'total')::decimal(19,4), 0) / invltotal.invl_total), 4)
    ...
    CASE WHEN fintrainv.transaction_amount IS NULL
        ...
        0.0000
    ELSE
        round(fintrainv.transaction_amount::decimal(19,4) * (coalesce(jsonb_extract_path_text(invl.jsonb, 'total')::decimal(19,4), 0) / invltotal.invl_total), 4)
...
GROUP BY
    ...
    jsonb_extract_path_text(invl.jsonb, 'total')::decimal(19,4),
