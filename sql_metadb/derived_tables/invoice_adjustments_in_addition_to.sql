DROP TABLE IF EXISTS folio_derived.invoice_adjustments_in_addition_to;

-- This query will return the invoice adjustments when they are "Not Prorated" and "In addition To"
CREATE TABLE folio_derived.invoice_adjustments_in_addition_to AS
WITH adjustments AS (
    SELECT
        id AS invoice_id,
...
CREATE INDEX ON folio_derived.invoice_adjustments_in_addition_to (invoice_id);

CREATE INDEX ON folio_derived.invoice_adjustments_in_addition_to (adjustment_description);

CREATE INDEX ON folio_derived.invoice_adjustments_in_addition_to (adjustment_prorate);

CREATE INDEX ON folio_derived.invoice_adjustments_in_addition_to (adjustment_relationToTotal);

CREATE INDEX ON folio_derived.invoice_adjustments_in_addition_to (adjustment_type);

CREATE INDEX ON folio_derived.invoice_adjustments_in_addition_to (adjustment_value);
