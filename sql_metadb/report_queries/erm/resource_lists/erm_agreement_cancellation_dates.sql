...
internal_contacts AS (
    SELECT
        ...
        jsonb_extract_path_text(users.jsonb, 'personal', 'lastName') || ', ' || jsonb_extract_path_text(users.jsonb, 'personal', 'firstName') AS names
    FROM
        folio_agreements.internal_contact
        LEFT JOIN folio_users.users ON jsonb_extract_path_text(users.jsonb, 'id')::uuid = internal_contact.ic_user_fk
)
SELECT
